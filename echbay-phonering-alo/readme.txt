=== EchBay Phonering Alo ===
Plugin Name: EchBay Phonering Alo
Plugin URI: https://www.facebook.com/groups/wordpresseb
Plugin Facebook page: https://www.facebook.com/webgiare.org
Author: Dao Quoc Dai
Author URI: https://www.facebook.com/ech.bay
Text Domain: echbayepa
Tags: phonering, call, sales, customers, contact
Requires at least: 4.8
Tested up to: 6.6
Stable tag: 1.3.0
Version: 1.3.0
Contributors: itvn9online
Donate link: https://paypal.me/itvn9online/5

== Description ==

Add Phonering Alo button to your website. A very simple yet very effective plugin that adds a Call Now button to your website for every device (mobile, table and desktop).

== Installation ==
1. Upload `echbay-phonering-alo` folder to the `/wp-content/plugins/` directory
2. Activate the plugin through the `Plugins` admin menu in WordPress
3. Go to menu `Webgiare Plugins` menu and `Phonering Alo` menu in WordPress for custom setting.

== Frequently Asked Questions ==

== Screenshots ==
1. Phonering Alo display.
2. Phonering Alo setting in admin.

== Changelog ==

= Version 1.3.0 =
* fixed url tel phone

= Version 1.2.9 =
* new css for button call number only

= Version 1.2.8 =
* new style for button call number only

= Version 1.2.7 =
* add class for custom tracking via gooogle tagmanager

= Version 1.2.6 =
* add ON/ OFF tracking for sms, zalo and mes

= Version 1.2.5 =
* ON/ OFF tracking

= Version 1.2.4 =
* Fixed get current time

= Version 1.2.3 =
* Support default for Echbaydotcom plugin.

= Version 1.2.2 =
* Get time for reset cache.

= Version 1.2.1 =
* Auto reset cache after 10 minute.

= Version 1.2.0 =
* Save HTML to cache file and using.

= Version 1.1.9 =
* Custom setup full URL via m.me or goo.gl for tracking vist to messenger

= Version 1.1.8 =
* Add parameter for tracking messenger source

= Version 1.1.7 =
* Add zalo chat for Vietnam user

= Version 1.1.6 =
* Google analytics tracking

= Version 1.1.5 =
* Fixed css for mobile

= Version 1.1.4 =
* Position for new template

= Version 1.1.2 =
* Add new template for phonering...

= Version 1.1.1 =
* Fixed preview in admin

= Version 1.1.0 =
* Add option for using Font Awesome icons

= Version 1.0.9 =
* Preview change in admin, and set custom color for border and background

= Version 1.0.8 =
* Fixed click if phone number is null

= Version 1.0.6 =
* position center for ring phone button

= Version 1.0.5 =
* Update style for pc

= Version 1.0.4 =
* Up for WP 4.9

= Version 1.0.3 =
* Custom select device for active plugin

= Version 1.0.2 =
* Fixed dafault value if zero

= Version 1.0.1 =
* Show or hidden button phone alo in desktop

= Version 1.0.0 =
* None

== Upgrade Notice ==

= Version 1.2.3 =
* None

= Version 1.2.2 =
* None

= Version 1.2.1 =
* None

= Version 1.2.0 =
* None

= Version 1.1.9 =
* None

= Version 1.1.8 =
* None

= Version 1.1.7 =
* None

= Version 1.1.6 =
* None

= Version 1.1.5 =
* None

= Version 1.1.4 =
* None

= Version 1.1.2 =
* None

= Version 1.1.1 =
* None

= Version 1.1.0 =
* None

= Version 1.0.9 =
* None

= Version 1.0.8 =
* None

= Version 1.0.6 =
* None

= Version 1.0.5 =
* None

= Version 1.0.4 =
* None

= Version 1.0.3 =
* None

= Version 1.0.2 =
* None

= Version 1.0.1 =
* None

= Version 1.0.0 =
* None
